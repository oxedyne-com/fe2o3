...
/// C++ token definitions.
pub fn cpp_tokens() -> LangTokens {
    let keywords: BTreeSet<String> = [
        // C keywords carried forward.
        "auto", "break", "case", "char", "const", "continue", "default",
        "do", "double", "else", "enum", "extern", "float", "for", "goto",
        ...
        "true", "try", "typeid", "typename", "using", "virtual", "xor",
...
/// C# token definitions.
pub fn csharp_tokens() -> LangTokens {
    let keywords: BTreeSet<String> = [
        ...
        "volatile", "while",
        // Contextual keywords.
        "async", "await", "get", "set", "value", "yield", "partial",
        "where", "nameof", "when", "init", "record", "required",
    ].iter().map(|s| s.to_string()).collect();
...
pub fn lex(src: &str, lang: &LangTokens) -> Outcome<Vec<Token>> {
    ...
    loop {
        ...
        loop {
            ...
            // In Rust, `///` is a doc comment but `////` is a regular
            // line comment. Reject prefixes that are followed by the
            // same delimiter character (e.g. `/` after `///`).
            let mut is_doc = false;
            for prefix in &lang.doc_comment_prefixes {
                if src[pos..].starts_with(prefix) {
                    let after = pos + prefix.len();
                    if !prefix.is_empty() && after < len {
                        let delim = prefix.as_bytes()[prefix.len() - 1];
                        if bytes[after] == delim {
                            continue;
                        }
                    }
                    let start = pos;
