...
pub fn test_daticle_func(filter: &'static str) -> Outcome<()> {
    ...
    res!(test_it(filter, &["Simple dat", "all", "i32"], || {
        ...
        let n = res!(match d {
            Dat::I32(n) => Ok(n),
            _ => Err(err!("A Dat::I32 was expected, found {:?}", d; Input, Conversion, Mismatch)),
        });
        req!(n, 42i32);
        Ok(())
    }));
        ...
        req!(
            ...
            match std::mem::size_of::<usize>() {
                ...
                s   => return Err(err!(
                    ...
                    mapped to a daticle kind.", s; System, Unimplemented, Bug)),
            },
        );
        Ok(())
    }));
    ...
    res!(test_it(filter, &["From 250", "all", "from", "vek"], || {
        ...
        match Dat::try_vek_from(v.clone()) {
            Ok(_) => return Err(err!(
                    "Creation of {:?} should have detected different kind in {:?}.",
                    Kind::Vek, v; Test)),
    ...
    res!(test_it(filter, &["From 260", "all", "from", "bu64"], || {
    	req!(Dat::from(vec![0u8]).kind(), Kind::BU64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 261", "all", "from", "best", "bu64"], || {
    	req!(Dat::best_from(vec![0u8]).kind(), Kind::BU64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 262", "all", "from", "bu64"], || {
    	req!(Dat::from(&vec![0u8][..]).kind(), Kind::BU64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 263", "all", "from", "best", "bu64"], || {
    	req!(Dat::best_from(&vec![0u8][..]).kind(), Kind::BU64);
        Ok(())
    }));

    // BU8 -> BU32 and BC64 must be created manually

    // Fixed length bytes
    
    res!(test_it(filter, &["From 320", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 2]).kind(), Kind::B2);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 321", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 3]).kind(), Kind::B3);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 322", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 4]).kind(), Kind::B4);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 323", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 5]).kind(), Kind::B5);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 324", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 6]).kind(), Kind::B6);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 325", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 7]).kind(), Kind::B7);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 326", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 8]).kind(), Kind::B8);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 327", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 9]).kind(), Kind::B9);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 328", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 10]).kind(), Kind::B10);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 329", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 16]).kind(), Kind::B16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 330", "all", "from", "tuple"], || {
    	req!(Dat::from([0u8; 32]).kind(), Kind::B32);
        Ok(())
    }));

    // Fixed length numbers
    
    res!(test_it(filter, &["From 340", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 2]).kind(),  Kind::Tup2u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 341", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 3]).kind(),  Kind::Tup3u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 342", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 4]).kind(),  Kind::Tup4u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 343", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 5]).kind(),  Kind::Tup5u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 344", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 6]).kind(),  Kind::Tup6u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 345", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 7]).kind(),  Kind::Tup7u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 346", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 8]).kind(),  Kind::Tup8u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 347", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 9]).kind(),  Kind::Tup9u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 348", "all", "from", "tuple"], || {
    	req!(Dat::from([0u16; 10]).kind(), Kind::Tup10u16);
        Ok(())
    }));

    
    res!(test_it(filter, &["From 350", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 2]).kind(),  Kind::Tup2u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 351", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 3]).kind(),  Kind::Tup3u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 352", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 4]).kind(),  Kind::Tup4u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 353", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 5]).kind(),  Kind::Tup5u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 354", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 6]).kind(),  Kind::Tup6u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 355", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 7]).kind(),  Kind::Tup7u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 356", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 8]).kind(),  Kind::Tup8u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 357", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 9]).kind(),  Kind::Tup9u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 358", "all", "from", "tuple"], || {
    	req!(Dat::from([0u32; 10]).kind(), Kind::Tup10u32);
        Ok(())
    }));

    
    res!(test_it(filter, &["From 360", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 2]).kind(),  Kind::Tup2u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 361", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 3]).kind(),  Kind::Tup3u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 362", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 4]).kind(),  Kind::Tup4u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 363", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 5]).kind(),  Kind::Tup5u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 364", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 6]).kind(),  Kind::Tup6u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 365", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 7]).kind(),  Kind::Tup7u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 366", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 8]).kind(),  Kind::Tup8u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 367", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 9]).kind(),  Kind::Tup9u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 368", "all", "from", "tuple"], || {
    	req!(Dat::from([0u64; 10]).kind(), Kind::Tup10u64);
        Ok(())
    }));

    
    res!(test_it(filter, &["From 420", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 2]).kind(), Kind::B2);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 421", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 3]).kind(), Kind::B3);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 422", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 4]).kind(), Kind::B4);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 423", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 5]).kind(), Kind::B5);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 424", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 6]).kind(), Kind::B6);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 425", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 7]).kind(), Kind::B7);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 426", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 8]).kind(), Kind::B8);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 427", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 9]).kind(), Kind::B9);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 428", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 10]).kind(), Kind::B10);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 429", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 16]).kind(), Kind::B16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 430", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u8; 32]).kind(), Kind::B32);
        Ok(())
    }));

    
    res!(test_it(filter, &["From 440", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 2]).kind(),  Kind::Tup2u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 441", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 3]).kind(),  Kind::Tup3u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 442", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 4]).kind(),  Kind::Tup4u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 443", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 5]).kind(),  Kind::Tup5u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 444", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 6]).kind(),  Kind::Tup6u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 445", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 7]).kind(),  Kind::Tup7u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 446", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 8]).kind(),  Kind::Tup8u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 447", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 9]).kind(),  Kind::Tup9u16);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 448", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u16; 10]).kind(), Kind::Tup10u16);
        Ok(())
    }));

    
    res!(test_it(filter, &["From 450", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 2]).kind(),  Kind::Tup2u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 451", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 3]).kind(),  Kind::Tup3u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 452", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 4]).kind(),  Kind::Tup4u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 453", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 5]).kind(),  Kind::Tup5u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 454", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 6]).kind(),  Kind::Tup6u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 455", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 7]).kind(),  Kind::Tup7u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 456", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 8]).kind(),  Kind::Tup8u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 457", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 9]).kind(),  Kind::Tup9u32);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 458", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u32; 10]).kind(), Kind::Tup10u32);
        Ok(())
    }));

    
    res!(test_it(filter, &["From 460", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u64; 2]).kind(),  Kind::Tup2u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 461", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u64; 3]).kind(),  Kind::Tup3u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 462", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u64; 4]).kind(),  Kind::Tup4u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 463", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u64; 5]).kind(),  Kind::Tup5u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 464", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u64; 6]).kind(),  Kind::Tup6u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 465", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u64; 7]).kind(),  Kind::Tup7u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 466", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u64; 8]).kind(),  Kind::Tup8u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 467", "all", "from", "best", "tuple"], || {
    	req!(Dat::best_from([0u64; 9]).kind(),  Kind::Tup9u64);
        Ok(())
    }));
    
    res!(test_it(filter, &["From 468", "all", "from", "best", "tuple"], || {
        req!(Dat::best_from([0u64; 10]).kind(), Kind::Tup10u64);
        Ok(())
    }));
