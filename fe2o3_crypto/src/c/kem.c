...
int crypto_kem_keypair(unsigned char *pk, unsigned char *sk)
{
  ...
  saber_sha3_256(sk + SABER_SECRETKEYBYTES - 64, pk, SABER_INDCPA_PUBLICKEYBYTES); // Then hash(pk) is appended.
...
int crypto_kem_enc(unsigned char *c, unsigned char *k, const unsigned char *pk)
{
  ...
  saber_sha3_256(buf, buf, 32); // BUF[0:31] <-- random message (will be used as the key for client) Note: hash doesnot release system RNG output

  saber_sha3_256(buf + 32, pk, SABER_INDCPA_PUBLICKEYBYTES); // BUF[32:63] <-- Hash(public key);  Multitarget countermeasure for coins + contributory KEM

  saber_sha3_512(kr, buf, 64);               // kr[0:63] <-- Hash(buf[0:63]);
                                       // K^ <-- kr[0:31]
                                       // noiseseed (r) <-- kr[32:63];
  indcpa_kem_enc(buf, kr + 32, pk, c); // buf[0:31] contains message; kr[32:63] contains randomness r;

  saber_sha3_256(kr + 32, c, SABER_BYTES_CCA_DEC);

  saber_sha3_256(k, kr, 64); // hash concatenation of pre-k and h(c) to k
...
int crypto_kem_dec(unsigned char *k, const unsigned char *c, const unsigned char *sk)
{
  ...
  saber_sha3_512(kr, buf, 64);

  indcpa_kem_enc(buf, kr + 32, pk, cmp);

  fail = verify(c, cmp, SABER_BYTES_CCA_DEC);

  saber_sha3_256(kr + 32, c, SABER_BYTES_CCA_DEC); // overwrite coins in kr with h(c)
  //hexdump("WEE ", kr, 32, 64);
