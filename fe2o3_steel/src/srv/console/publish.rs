...
mod ui_dump {
	...
	fn ui_dump_the_console_screens() -> Outcome<()> {
		...
		// The author row of a post written under another identity, which is the one state the
		// composer dump above cannot reach: a new post is always the composer's own.
		// Rendered inside the field row the composer really puts it in, so the photograph shows the
